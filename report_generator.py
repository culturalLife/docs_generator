...
def _assemble_docx(content: str, output_path: str, request_id: str = ""):
    ...
    # Title page
    p = doc.add_paragraph("InvestSLM", style='ReportTitle')
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    sub = doc.add_paragraph("Domain AI Model Tuning Decision Framework — Final Report", style='ReportSubtitle')
    sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph("\nPrepared for: US Based Investment Bank")
    doc.add_paragraph("Prepared by: AI COE")
    doc.add_paragraph("Status: Final Board Review\n")
    doc.add_page_break()
...
def generate_report(
    ...
    # Section definitions — one entry per parallel call
    # Part number must match the stitching order below
    parts = [
        (
            1,
            "Sections 1, 2, 3, and 4 (Executive Summary, Strategic Recommendation, "
            "Domain Niche-ness & Hallucination Risk Analysis, Competitive Moat Analysis)",
        ),
        (
            2,
            "Sections 5, 6, and 7 (Recommended Training Method Stack, "
            "Data Contract Summary, Tech Platform & Base Model Recommendation)",
        ),
        (
            3,
            "Sections 8, 9, 10, 11, 12, and 13 (Execution Roadmap with Named Responsibilities, "
            "Risks, Open Items & Mitigations, Success Criteria & Acceptance Definition, "
            "Primary Consulting Observations, Final Recommendation & Confidence Level, What Could Go Wrong)",
        ),
    ]
    ...
    stitched_content = "\n\n".join([results[1], results[2], results[3]])
